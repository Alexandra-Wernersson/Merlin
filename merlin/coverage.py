...
def run_coverage_test(trainer, network, store_samples, config, output_path,
    ...
    Checks whether posterior credible intervals are correctly calibrated
    (e.g. the true parameter should fall inside the 68% credible region
    ~68% of the time) — a standard SBI diagnostic, since over/under-confident
    posteriors are a known failure mode of neural ratio/posterior estimators.

    The test set is the last `n_test` simulations in store_samples. Prior
    samples for comparison are drawn from the same Simulator/PriorSampler as
    training, to keep the comparison prior consistent across all parameters
    (including Gaussian-distributed photo-z dimensions). If any inferred
    ...
    for i in range(N_plot):
        ...
        # swyft.plot_zz hardcodes fontsize=14 for its in-panel annotations;
        # no fontsize parameter is exposed, so bump post-hoc.
        for txt in ax.texts:
            txt.set_fontsize(14)
        ax.set_title(PARAM_LABELS.get(param_names[i], param_names[i]), fontsize=21, pad=8)
        ax.tick_params(axis="both", which="major", labelsize=13)
        # Bottom-most VISIBLE panel per column (last row may be ragged, e.g.
        # 13 params over 5 cols) gets the x-label; leftmost column gets the
        # y-label — geometry-aware version of label_outer().
        if i + n_cols >= N_plot:
            ax.set_xlabel("Nominal credibility ($z_p$)", fontsize=16)
        else:
            # tick_params only hides tick values, not plot_zz's default
            # xlabel text, so clear it explicitly too.
            ax.set_xlabel("")
